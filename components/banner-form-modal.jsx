...
import { getBannerById, addBanner, updateBanner } from "@/services/banner";
const formSchema = z.object({
  image: z.any().refine((file) => file && file.size > 0, "Image is required"),
});
    ...
    if (open) {
      if (mode === "add") {
        ...
        const fetchBannerDetails = async () => {
          try {
            setLoading(true);
            const response = await getBannerById(banner._id);
